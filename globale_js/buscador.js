...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Buscador con imágenes cargado');
...
async function buscarJuegoExacto(termino) {
    console.log('🔍 Buscando juego:', termino);
    
    try {
        const url = window.location.href.toLowerCase();
        let consolaActual = 'ps2';
        if (url.includes('ps3')) consolaActual = 'ps3';
        if (url.includes('ps4')) consolaActual = 'ps4';
        
        const todosJuegos = await cargarTodosJuegos(consolaActual);
        const terminoLower = termino.toLowerCase();
        ...
        if (juegoEncontrado) {
            ...
            const numero = consolaActual.slice(-1);
            ...
            buscarEnOtrasConsolasExacto(termino, consolaActual);
        }
...
async function buscarEnOtrasConsolasExacto(termino, consolaActual) {
    const otrasConsolas = ['ps2', 'ps3', 'ps4'].filter(c => c !== consolaActual);
    
    for (const consola of otrasConsolas) {
        try {
            const juegos = await cargarTodosJuegos(consola);
            const terminoLower = termino.toLowerCase();
            
            const juegoEncontrado = juegos.find(juego => {
                const nombreJuego = (juego.Nombre || juego.nombre || '').toLowerCase();
                return nombreJuego.includes(terminoLower);
            });
            ...
            if (juegoEncontrado) {
                const itemsPorPagina = 12;
                const indexJuego = juegos.findIndex(j => 
                    (j.Nombre || j.nombre).toLowerCase() === (juegoEncontrado.Nombre || juegoEncontrado.nombre).toLowerCase()
                );
                const paginaDelJuego = Math.floor(indexJuego / itemsPorPagina) + 1;
    ...
    mostrarMensaje(`"${termino}" no encontrado en ninguna consola`);
}
...
// INYECTAR ESTILOS CSS CON IMÁGENES
